---
title: "Koichi_IDHm_Metabolic_analysis"
...
```{r}
IDHm_pheno <- colnames(mIDHi_react_scores_GEM) %>% sapply(function(sample){
...
mIDHi_react_scores_GEM <- mIDHi_react_scores_GEM[,IDHm_pheno == "IDH1" | IDHm_pheno == "IDH2"]
IDHm_pheno <- IDHm_pheno[IDHm_pheno == "IDH1" | IDHm_pheno == "IDH2"]
```

```{r}
res.pca <- prcomp(t(mIDHi_react_scores_GEM))
p <- fviz_pca_ind(res.pca, label="all", habillage=IDHm_pheno,
                               addEllipses=T, ellipse.level=0.95)
p <- p + ggtitle("PCA reactions activities")
p
png("../Results/Metabolism/PCA_Koichi_metabolic_reaction_data_IDHm.png")
p
dev.off()
```
...
Metabolic_network <- mclapply(names(Metabo_reactions), function(reaction){
...
write.table(Metabolic_network_clean, "~/GitHub/Thesis_paper/Results/Metabolism/Metabolic_network_clean.tsv", sep = "\t", row.names = F, quote = F)
```
...
Metabolic_network_clean_KOICHI_IDHm_filtered$B <- stringr::str_replace_all(Metabolic_network_clean_KOICHI_IDHm_filtered$B, pattern = "'", replacement = "p")
