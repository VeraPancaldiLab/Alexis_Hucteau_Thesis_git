...
```{r}
Classes <- list.dirs(Main_folder, full.names = F, recursive = F) %>% .[!stringr::str_detect(., "Comparison") & !stringr::str_detect(., "Results")]
Classes
```
...
Plot_best_flows <- function(Flow_1, Flow_2){
  ...
  return(list(Flow_data = flows, Plot = NA))
}
...
```{r}
Flow_analysis <- lapply(names(Comparisons), function(Comparison){
...
```{r}
Results_flow_analysis <- "~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/"
...
```{r}
Flow_analysis <- lapply(names(Comparisons), function(comp){
  read.table(paste0(Results_flow_analysis, comp, "_Comparison/", comp, "_flow_data_2.tsv"), sep = "\t", header = T)
})
names(Flow_analysis)<-names(Comparisons)
```
...
Compare_Enrichments <- function(Comparison_flow, missing = T, only_up = F, only_down = F){
  list_DFG <- lapply(names(Comparison_flow), function(comp){
    ...
    df <- dplyr::filter(df, !stringr::str_detect(name, "chr") & 
                          !stringr::str_detect(name, "MAR0") & 
                          !stringr::str_detect(name, "MAR1") & 
                          !stringr::str_detect(name, "MIR") & name %in% genes$hgnc_symbol)
  
    df$missing_values <- (df[2] == 0 | df[3] == 0)
    Mean_distance_up <- mean(df$distance[df$distance > 0])
    q1_up <- quantile(df$distance[df$distance > 0])[2]
    q3_up <- quantile(df$distance[df$distance > 0])[4]
    high_distance_up <- q3_up + 1.5*(q3_up-q1_up) 
    
    Mean_distance_down <- mean(df$distance[df$distance < 0])
    q1_down <- quantile(df$distance[df$distance < 0])[4]
    q3_down <- quantile(df$distance[df$distance < 0])[2]
    high_distance_down <-  + q3_down - 1.5*(q1_down-q3_down)
    
    if(!missing){
      df <- dplyr::filter(df, missing_values == T)
    }
    up <- dplyr::filter(df, distance > high_distance_up) %>% .$name
    down <- dplyr::filter(df, distance < high_distance_down) %>% .$name
    if(only_up){
      down <- up
    }else if(only_down){
      up <- down
  ...
  go_cluster <- compareCluster(Entrez~group, data = mydf, fun = "enrichGO", ont = "BP", OrgDb = org.Hs.eg.db, 
...
Cluster_enrichment_GO_down <- Compare_Enrichments(Flow_analysis, only_up = F, only_down = T, missing = T)
...
```{r}
Make_list_ID <- function(Flow_comparison, pos = T, missing = T){
  df <- Flow_comparison[order(abs(Flow_comparison$distance), decreasing = T),]
  df <- dplyr::filter(df, name %in% genes$hgnc_symbol)
  
  df$missing_values <- (df[2] == 0 | df[3] == 0)
  if(!missing){
    df <- dplyr::filter(df, missing_values == F)
  }
  if(pos){
    Mean_distance <- mean(df$distance[df$distance > 0])
    q1 <- quantile(df$distance[df$distance > 0])[2]
    q3 <- quantile(df$distance[df$distance > 0])[4]
    high_distance <- q3 + 1.5*(q3-q1)
    ids <- dplyr::filter(df, distance > high_distance) %>% .$name
  }else{
    Mean_distance <- mean(df$distance[df$distance < 0])
    q1 <- quantile(df$distance[df$distance < 0])[4]
    q3 <- quantile(df$distance[df$distance < 0])[2]
    high_distance <-  q3 - 1.5*(q1-q3)
    ids <- dplyr::filter(df, distance < high_distance) %>% .$name
  }
  ids
}
...
```{r}
List_id_IDHs <- list(
  "IDH1 in Responder" = Make_list_ID(Flow_analysis$IDH_in_Responder, pos = F),
  "IDH1 in Non Responder" = Make_list_ID(Flow_analysis$IDH_in_Non_Responders, pos = F),
  "IDH2 in Responder" = Make_list_ID(Flow_analysis$IDH_in_Responder, pos = T),
  "IDH2 in Non Responder" = Make_list_ID(Flow_analysis$IDH_in_Non_Responders, pos = T)
)
List_id_Response <- list(
  "Responder in IDH1" = Make_list_ID(Flow_analysis$Response_in_IDH1, pos = F),
  "Responder in IDH2" = Make_list_ID(Flow_analysis$Response_in_IDH2, pos = F),
  "Non Responder in IDH1" = Make_list_ID(Flow_analysis$Response_in_IDH1, pos = T),
  "Non Responder in IDH2" = Make_list_ID(Flow_analysis$Response_in_IDH2, pos = T)
)

List_id_IDH_only <- list(
  "IDH1" = Make_list_ID(Flow_analysis$mIDH, pos = F),
  "IDH2" = Make_list_ID(Flow_analysis$mIDH, pos = T)
)

List_id_Response_only <-list(
  "Responder" = Make_list_ID(Flow_analysis$Response, pos = F),
  "Non Responder" = Make_list_ID(Flow_analysis$Response, pos = T)
)

df_IDH <- names(List_id_IDHs) %>% lapply(function(ids){
  data.table(Entrez = List_id_IDHs[[ids]], group = ids)
}) %>% data.table::rbindlist()

df_Response <- names(List_id_Response) %>% lapply(function(ids){
  data.table(Entrez = List_id_Response[[ids]], group = ids)
}) %>% data.table::rbindlist()

df_IDH_only <- names(List_id_IDH_only) %>% lapply(function(ids){
  data.table(Entrez = List_id_IDH_only[[ids]], group = ids)
}) %>% data.table::rbindlist()

df_Response_only <- names(List_id_Response_only) %>% lapply(function(ids){
  data.table(Entrez = List_id_Response_only[[ids]], group = ids)
}) %>% data.table::rbindlist()

Compare_IDH <- compareCluster(Entrez~group, data = df_IDH, fun = "enrichGO", ont = "BP", OrgDb = org.Hs.eg.db, 
               keyType = "SYMBOL", pvalueCutoff = 0.1, pAdjustMethod = "none", 
               qvalueCutoff = 1, universe = universe_hgnc)

Compare_Response <- compareCluster(Entrez~group, data = df_Response, fun = "enrichGO", ont = "BP", OrgDb = org.Hs.eg.db, 
               keyType = "SYMBOL", pvalueCutoff = 0.1, pAdjustMethod = "none", 
               qvalueCutoff = 1, universe = universe_hgnc)

# Compare_IDH_only <- compareCluster(Entrez~group, data = df_IDH_only, fun = "enrichGO", ont = "BP", OrgDb = org.Hs.eg.db, 
#                keyType = "SYMBOL", pvalueCutoff = 0.1, pAdjustMethod = "none", 
#                qvalueCutoff = 1, universe = universe_hgnc)
# 
# Compare_Response_only <- compareCluster(Entrez~group, data = df_Response_only, fun = "enrichGO", ont = "BP", OrgDb = org.Hs.eg.db, 
#                keyType = "SYMBOL", pvalueCutoff = 0.1, pAdjustMethod = "none", 
#                qvalueCutoff = 1, universe = universe_hgnc)
...
```{r}
TOP_100_Response_only <- Flow_analysis[["Response"]] %>% .[order(.$distance, decreasing = F),] %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% .$name %>% .[1:100]
TOP_100_Response_only

TOP_100_Non_Response_only <- Flow_analysis[["Response"]] %>% .[order(.$distance, decreasing = T),] %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% .$name %>% .[1:100]
TOP_100_Non_Response_only

TOP_100_IDH1_only <- Flow_analysis[["mIDH"]] %>% .[order(.$distance, decreasing = F),] %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% .$name %>% .[1:100]
TOP_100_IDH1_only

TOP_100_IDH2_only <- Flow_analysis[["mIDH"]] %>% .[order(.$distance, decreasing = T),] %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% .$name %>% .[1:100]
TOP_100_IDH2_only
```
...
```{r}
n <- 10
p <- dotplot(Compare_IDH, showCategory = n) 
n_comp <- 4
height_param <- n*n_comp*120 +200
width_param <- n_comp*900 + 300
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_IDH_plot.png", 
       p, bg = "white", width = width_param, 
       height = height_param, units = "px", limitsize = FALSE)

pcnet <- cnetplot(Compare_IDH, showCategory = n, cex_label_gene = 2, cex_label_category = 2.5) 
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_IDH_cnetplot.png", pcnet, bg = "white", width = 5700, height = 5700, units = "px", limitsize = FALSE)
```


```{r}
n <- 10
p <- dotplot(Compare_Response, showCategory = n) 
n_comp <- 4
height_param <- n*n_comp*120 +200
width_param <- n_comp*900 + 300
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_Response_plot.png", 
       p, bg = "white", width = width_param, 
       height = height_param, units = "px", limitsize = FALSE)

pcnet <- cnetplot(Compare_Response, showCategory = n, cex_label_gene = 2, cex_label_category = 2.5) 
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_Response_cnetplot.png", pcnet, bg = "white", width = 5700, height = 5700, units = "px", limitsize = FALSE)
```


```{r}
n <- 30
p <- dotplot(Compare_IDH_only, showCategory = n) 
n_comp <- 2
height_param <- n*n_comp*120 +200
width_param <- n_comp*900 + 300
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Compare_IDH_only_plot.png", 
       p, bg = "white", width = width_param, 
       height = height_param, units = "px", limitsize = FALSE)

pcnet <- cnetplot(Compare_IDH_only, showCategory = n, cex_label_gene = 2, cex_label_category = 2) 
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Compare_IDH_only_cnetplot.png", pcnet, bg = "white", width = 5700, height = 5700, units = "px",  limitsize = FALSE)
```
...
n <- 30
n_comp <- 2
p <- dotplot(Compare_Response_only, showCategory = n) 
height_param <- n*n_comp*120 +200
width_param <- n_comp*900 + 300
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_Response_only_plot.png", 
       p, bg = "white", width = width_param, 
       height = height_param, units = "px", limitsize = FALSE)

pcnet <- cnetplot(Compare_Response_only, showCategory = n, cex_label_gene = 2, cex_label_category = 2.5) 
ggsave("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/Cluster_Response_only_cnetplot.png", pcnet, bg = "white",  width = 5700, height = 5700, units = "px",  limitsize = FALSE)
```
...
Do_gene_ontology_Kegg_analysis <- function(Flow_data, Distance_threshold, Title){
  ...
  both_side <- genes_down <- Flow_data %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% unique
  ...
  enrichments_both_side <- Do_enrichment_analysis(genes_ensembl_both_c, paste(Title, "GSEA"))
  enrichments_both_side
}
```
...
DO_all_enrichments <- function(Res_Data, Flow_threshold, Title){
  genes_flow_data <- Flow_data %>% dplyr::filter(name %in% genes$hgnc_symbol) %>% unique
...
GO_Analysis <- lapply(names(Flow_analysis), function(Class){
  Do_gene_ontology_Kegg_analysis(Flow_analysis[[Class]], 0.005, Class)
})
...
```{r}
GO_Analysis %>% saveRDS(file = "../../Results/Multi_layer_pheno_like/Final_mutlilayer_results/GO_analysis_06_12.rds")
```
...
Cnetplot <- mclapply(names(GO_Analysis), function(Comparisons){
  pcnet <- lapply(names(GO_Analysis[[Comparisons]]), function(Ontology){
    if(!is.null(GO_Analysis[[Comparisons]][[Ontology]][["Cnetplot"]])){
      # cnet <- GO_Analysis[[Comparisons]][[Ontology]][["Cnetplot"]]
      # barplot <- GO_Analysis[[Comparisons]][[Ontology]][["Enrichment_barplot"]]
      dotplot <- GO_Analysis[[Comparisons]][[Ontology]][["Enrichment_dotplot"]]
      # ggsave(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/", 
...
```{r}
mclapply(names(GO_Analysis), function(Comparisons){
  pcnet <- lapply(names(GO_Analysis[[Comparisons]]), function(Ontology){
    if(!is.null(GO_Analysis[[Comparisons]][[Ontology]][["Cnetplot"]])){
      GO_Analysis[[Comparisons]][[Ontology]][["Readable_enrichment"]]@result %>%
        write.table(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/", Comparisons,
                           "_Comparison/", Comparisons, "_", Ontology, "_table.tsv"))
    }
  }) 
}, mc.cores = cores2use)
```
...
path_2_analyse <- c("GO_Cellular_Component_2023", "GO_Molecular_Function_2023", "Reactome_2022", "KEGG_2021_Human", "WikiPathway_2023_Human")

path_2_analyse_BP <- c("GO_Biological_Process_2023")
```

```{r}
EnrichR_multi_analysis <- function(Comparison_flow, missing = T, only_up = F, only_down = F){
  list_DFG <- lapply(names(Comparison_flow), function(comp){
    df <- Comparison_flow[[comp]][["Flow_data"]][order(abs(Comparison_flow[[comp]][["Flow_data"]]$distance), decreasing = T),]
    df <- dplyr::filter(df, name %in% genes$hgnc_symbol)
    ...
    q1_up <- quantile(df$distance[df$distance > 0])[2]
    q3_up <- quantile(df$distance[df$distance > 0])[4]
    high_distance_up <- 1.5*(q3_up-q1_up) + q3_up
    
    Mean_distance_down <- mean(df$distance[df$distance < 0])
    q1_down <- quantile(df$distance[df$distance < 0])[4]
    q3_down <- quantile(df$distance[df$distance < 0])[2]
    high_distance_down <- -1.5*(q1_down-q3_down) + q3_down
  ...
  Enriched_list <- lapply(names(list_DFG), function(comparison){
    All_db <- enrichr(list_DFG[[comparison]], databases = path_2_analyse) %>%
      lapply(function(path){
        dplyr::filter(path, P.value < 0.05)
        }) %>% data.table::rbindlist()
    BP <- enrichr(list_DFG[[comparison]], databases = path_2_analyse_BP) %>%
      lapply(function(path){
        dplyr::filter(path, P.value < 0.05)
        }) %>% data.table::rbindlist()
    list("All_db" = All_db, "BP" = BP)
  }) 
    
  
  names(Enriched_list) <- names(list_DFG)

  list("Gene_ID_list" = list_DFG, "Enriched_list" = Enriched_list)
}
...
```{r}
EnrichRment_only_up <- EnrichR_multi_analysis(Flow_analysis, only_up = T)
EnrichRment_only_down <- EnrichR_multi_analysis(Flow_analysis, only_up = F, only_down = T)
```

```{r}
names(EnrichRment_only_up[["Enriched_list"]]) %>% lapply(function(comp){
  plotEnrich(EnrichRment_only_up[["Enriched_list"]][[comp]][["All_db"]], orderBy = "Adjusted.P.value", title = "", showTerms = 40, numChar = 60) %>%
    ggsave(filename = paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_up.png"), plot = .)
  EnrichRment_only_up[["Enriched_list"]][[comp]][["All_db"]] %>%
    write.table(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_up.tsv"))
  
  
  plotEnrich(EnrichRment_only_up[["Enriched_list"]][[comp]][["BP"]], orderBy = "Adjusted.P.value", title = "", showTerms = 40, numChar = 60) %>%
    ggsave(filename = paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_up_BP.png"), plot = .)
  EnrichRment_only_up[["Enriched_list"]][[comp]][["BP"]] %>%
    write.table(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_up_BP.tsv"))
})

names(EnrichRment_only_down[["Enriched_list"]]) %>% lapply(function(comp){
  plotEnrich(EnrichRment_only_down[["Enriched_list"]][[comp]][["All_db"]], orderBy = "Adjusted.P.value", title = "", showTerms = 40, numChar = 60) %>%
    ggsave(filename = paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_down.png"), plot = .)
  EnrichRment_only_down[["Enriched_list"]][[comp]][["All_db"]] %>%
    write.table(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_down.tsv"))
  
  
  plotEnrich(EnrichRment_only_down[["Enriched_list"]][[comp]][["BP"]], orderBy = "Adjusted.P.value", title = "", showTerms = 40, numChar = 60) %>%
    ggsave(filename = paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_down_BP.png"), plot = .)
  EnrichRment_only_down[["Enriched_list"]][[comp]][["BP"]] %>%
    write.table(paste0("~/GitHub/Thesis_paper/Results/Multi_layer_pheno_like/Final_mutlilayer_results/EnrichR_analysis/", comp, "_down_BP.tsv"))
})
```
...
```{r}
message("Response")
intersect(EnrichRment_only_down$Gene_ID_list$Response_in_IDH1, EnrichRment_only_down$Gene_ID_list$Response_in_IDH2)
message("Non Response")
intersect(EnrichRment_only_up$Gene_ID_list$Response_in_IDH1, EnrichRment_only_up$Gene_ID_list$Response_in_IDH2)
message("IDH1")
intersect(EnrichRment_only_down$Gene_ID_list$IDH_in_Responder, EnrichRment_only_down$Gene_ID_list$IDH_in_Non_Responders)
message("IDH2")
intersect(EnrichRment_only_up$Gene_ID_list$IDH_in_Responder, EnrichRment_only_up$Gene_ID_list$IDH_in_Non_Responders)
...
```{r}
message("Response")
intersect(EnrichRment_only_down$Enriched_list$Response_in_IDH2$BP$Term, EnrichRment_only_down$Enriched_list$Response_in_IDH1$BP$Term)

message("Non Response")
intersect(EnrichRment_only_up$Enriched_list$Response_in_IDH2$BP$Term, EnrichRment_only_up$Enriched_list$Response_in_IDH1$BP$Term)

message("IDH1")
intersect(EnrichRment_only_down$Enriched_list$IDH_in_Responder$BP$Term, EnrichRment_only_down$Enriched_list$IDH_in_Non_Responders$BP$Term)

message("IDH2")
intersect(EnrichRment_only_up$Enriched_list$IDH_in_Responder$BP$Term, EnrichRment_only_up$Enriched_list$IDH_in_Non_Responders$BP$Term)
```
