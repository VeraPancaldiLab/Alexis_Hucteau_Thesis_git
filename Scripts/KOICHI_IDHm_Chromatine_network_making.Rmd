---
title: "KOICHI_IDHm_Chromatine_network_making"
...
```{r}
dCpGs_KOICHI_IDHm <- read.table("../Results/DNAmethylation/dCpGs_KOICHI_IDHm.tsv", sep = "\t", header = T, row.names = 1)
Anno_EPIC <- read.table("/media/alexis/DATA/Illumina_manifest/infinium-methylationepic.tsv", sep = "\t", header = T)
dCpGs_KOICHI_IDHm_annotated <- merge(dCpGs_KOICHI_IDHm, Anno_EPIC, by.x = "ID", by.y = "Name")
dCpGs_KOICHI_IDHm_annotated$Strand <- ifelse(dCpGs_KOICHI_IDHm_annotated$Strand == "F", "+", "-")
dCPGs_filtered_KOICHI_IDHm_annotated <- dplyr::filter(dCpGs_KOICHI_IDHm_annotated, P.Value < 0.05 & abs(logFC) > 0.15)
dCpGs_KOICHI_IDHm_GRanges <- GRanges(
  seqnames = dCPGs_filtered_KOICHI_IDHm_annotated$CHR,
  ranges = IRanges(dCPGs_filtered_KOICHI_IDHm_annotated$MAPINFO, dCPGs_filtered_KOICHI_IDHm_annotated$MAPINFO +1),
  strand = dCPGs_filtered_KOICHI_IDHm_annotated$Strand,
  CpG = dCPGs_filtered_KOICHI_IDHm_annotated$ID
)
...
```{r}
overlaps <- findOverlaps(pchic_Granges, dCpGs_KOICHI_IDHm_GRanges)
dCpGs_KOICHI_IDHm_pchic <- data.frame(mcols(pchic_Granges[queryHits(overlaps),]),
                          data.frame(mcols(dCpGs_KOICHI_IDHm_GRanges[subjectHits(overlaps),]))) %>% unique()

pchic_net <- dplyr::filter(Pchic_edges, source %in% dCpGs_KOICHI_IDHm_pchic$ID | target %in% dCpGs_KOICHI_IDHm_pchic$ID)

dCPGs_filtered_KOICHI_IDHm_annotated <- merge(dCPGs_filtered_KOICHI_IDHm_annotated, dCpGs_KOICHI_IDHm_pchic, by.x = "ID", by.y = "CpG")
colnames(dCPGs_filtered_KOICHI_IDHm_annotated)[11] <- "fragment_ID"
write.table(dCPGs_filtered_KOICHI_IDHm_annotated, "../Results/DNAmethylation/dCpGs_KOICHI_IDHm_pchic.tsv", sep = "\t", row.names = F)
write.table(pchic_net, "../Results/Chromatine_part/pchic_KOICHI_IDHm_dCPGs.tsv", sep = "\t", row.names = F)
```
...
Promoter_fragment %>% write.table("../Results/Chromatine_part/Promoter_named_pchic_dCPGs_KOICHI_IDHm.tsv", sep = "\t", quote = F, row.names = F)
```

```{r}
Fragments_features <- merge(dCPGs_filtered_KOICHI_IDHm_annotated, pchic_gene_network, by.x = "fragment_ID", by.y = "source", all.x = T)
...
write.table(Fragments_features_promoter_fragments, "../Results/Chromatine_part/Features_fragment_methylation_KOICHI_IDHm.tsv", sep = "\t", quote = F, row.names = F)
```
